...
//        int hash = hash(key);//使用key生成哈希值
//        int i = indexFor(hash, table.length);//利用算法计算出要存储的位置
//        for (Entry<K,V> e = table[i]; e != null; e = e.next) {//若此处为空，则添加元素
//            Object k;
//            //若哈希值相等且key相等，那就替换，并返回原来的值
//            //哈希值不相等或者key不相等，就添加元素
...
//    }
//
//    private void putAllForCreate(java.util.Map<? extends K, ? extends V> m) {
...
//        //如果数组中存放的元素个数大于某一比重，并且要存储的位置不为空，就扩容
...
//        //目前要存储的位置是有值（链表）的
//        //先将链表取出，再通过Entry构造器将该链表挂到要存储元素的next属性下，最后再存入该位置
//        /*Entry(int h, K k, V v, Entry<K,V> n) {
//            value = v;
...
//        throws IOException
//    {
//        Iterator<java.util.Map.Entry<K,V>> i =
